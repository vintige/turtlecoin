...
# docker run -v $(pwd)/xariad:/var/lib/xariad -v $(pwd)/wallet:/home/xaria --rm -ti xaria:0.2.2
...
ARG XARIA_BRANCH=master
ENV XARIA_BRANCH=${XARIA_BRANCH}
...
RUN apt-get update && \
    apt-get install -y \
      ...
      librocksdb-dev && \
    git clone https://github.com/xaria-coin/xaria.git /src/xaria && \
    cd /src/xaria && \
    git checkout $XARIA_BRANCH && \
    mkdir build && \
    cd build && \
    cmake -DCMAKE_CXX_FLAGS="-g0 -Os -fPIC -std=gnu++11" .. && \
    make -j$(nproc) && \
    mkdir -p /usr/local/bin && \
    cp src/TurtleCoind /usr/local/bin/Xariad && \
...
RUN useradd -r -s /usr/sbin/nologin -m -d /var/lib/xariad xariad && \
    ...
    echo "s6-setuidgid xariad /usr/local/bin/TurtleCoind" >> /etc/services.d/xariad/run && \
