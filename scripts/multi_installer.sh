...
# a one line clone-and-compile for xaria:
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/xaria ]; then
            read -r -p "${1:-xaria directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old xaria directory" && echo
                    rm -rf "$PWD"/xaria
                    ;;
                *)
                    _fail "xaria directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir xaria
        git clone -b master -q https://github.com/xaria-coin/xaria xaria   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd xaria
    fi
}

_build_xaria() {
    _note "Building xaria from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/xaria-coin')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/xaria')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "xaria Multi_Installer v1.0 (pepperoni)"
_colorize green " Thankyou for choosing Xaria" && echo
...
_note "Look in 'xaria/build/src/' for the executible binaries. See 'https://github.com/xaria-coin/xaria' for more project support. Cowabunga!"
